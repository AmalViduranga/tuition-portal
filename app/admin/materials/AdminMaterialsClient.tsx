...
export default function AdminMaterialsClient({ initialMaterials, initialClasses }: { initialMaterials: Material[], initialClasses: Class[] }) {
  ...
  const filteredMaterials = materials.filter((mat) => {
    const cgName = Array.isArray(mat.class_groups) ? mat.class_groups[0]?.name : mat.class_groups?.name;
    return mat.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
    (cgName || "").toLowerCase().includes(searchQuery.toLowerCase());
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (mat: Material) => {
        const cgName = Array.isArray(mat.class_groups) ? mat.class_groups[0]?.name : mat.class_groups?.name;
        return <Badge variant="info">{cgName || "No class"}</Badge>;
      },
    },
    {
      key: "release_at",
